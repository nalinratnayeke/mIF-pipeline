...
3. Upsample InstanSeg cell + nuclei labels and export masks (`*_whole_cell.tiff`, `*_nuclear.tiff`, uint32)
...
- Export uses nearest-neighbor resize (`order=0`, `preserve_range=True`, `anti_aliasing=False`) to preserve integer IDs for both whole-cell and nuclear masks.
...
- `mask_export.nuclear_suffix` controls nuclear mask filename suffix (default `_nuclear.tiff`).
